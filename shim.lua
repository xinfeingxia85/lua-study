...
function _.indexOf(arr, sub)
	if isString(arr) then
		sub = tostr(sub)
		if '' ~= sub then 
			return string.find(arr, sub)
		end
	end
...
function _.lastIndexOf(arr, sub)
	if isString(arr) then
		sub = tostr(sub)
		if '' ~= sub then
			return string.find(arr, sub .. '$')
		end
	elseif isTable(arr) then
		local i = #arr
		while i ~= 0 do
			if arr[i] == sub then
				return i
...
function _.join(arr, sep)
	return table.concat(_.map(arr, tostr), sep)
end
...
function _.startWith(str, val)
	return 0 +1 == _.indexOf(str, val)
end

function _.endWith(str, val)
	return val == _.slice(str, _.len(val) - _.len(val) + 1)
end
--string end
